...
Logo en el correo (LIMITACIÓN IMPORTANTE): el conector de Gmail asigna un Content-ID aleatorio a los adjuntos en línea (no sirve `cid:`) y Gmail bloquea las imágenes en base64 (data URI). Por eso **el logo va por URL pública** con `<img src="https://nc.nadaestaperdido.lat/index.php/s/ZEQCPo2FT5K46D4/download" alt="NADA ESTÁ PERDIDO" width="200">` en la cabecera durazno. Si esa URL no estuviera disponible, usa como respaldo el wordmark "NADA ESTÁ PERDIDO" en texto (verde sobre durazno). Nunca intentes incrustar el logo por CID ni por base64: no funciona en Gmail.

**Plantilla lista para usar:** parte SIEMPRE del archivo [`plantilla-correo.html`](plantilla-correo.html) que está en esta misma carpeta de la skill. Ya trae la cabecera con el logo por URL, la paleta, la tipografía, la tarjeta de noticia (duplícala por cada noticia) y los bloques opcionales de "Rumores en circulación" y "Cobertura limitada". Solo reemplaza los marcadores `{{...}}`. Esto ahorra tiempo y créditos: no rediseñes el HTML desde cero cada día.
